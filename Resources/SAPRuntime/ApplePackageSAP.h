...
// Read-only ZIP, decompressed in memory and hash-checked. Missing/corrupt resources never trigger a download.
uint64_t apsap_create_bundled(const uint8_t *hardware, uint64_t count, const char *resource_archive, char **error);
