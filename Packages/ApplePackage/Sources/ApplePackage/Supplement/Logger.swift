...
public enum APLogger {
    ...
    static func logRequest(method: String, url: String, headers: [(String, String)] = []) {
        ...
        for (name, value) in headers {
            let safeValue = redactedHeader(name, value: value)
            msg += "\n    \(name): \(safeValue)"
        }
        debug(msg)
    }
    ...
    static func logResponse(status: UInt, headers: [(String, String)] = [], bodySize: Int? = nil) {
        ...
        for (name, value) in headers {
            msg += "\n    \(name): \(redactedHeader(name, value: value))"
        }
        debug(msg)
    }

    static func redactedHeader(_ name: String, value: String) -> String {
        let name = name.lowercased()
        return ["token", "password", "cookie", "signature", "authorization"].contains(where: name.contains) ? "<redacted>" : value
    }
}
